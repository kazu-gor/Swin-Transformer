...
# TODO
class PatchSelectionTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 pretrain_img_size=224, patch_size=4, in_chans=3,
        ...
        decoder_layer_stage1 = TransformerDecoderLayer(
                d_model, nhead, dim_feedforward,
                dropout, activation, normalize_before
        )
        decoder_layer_stage2 = TransformerDecoderLayer(
                d_model*2, nhead, dim_feedforward,
                dropout, activation, normalize_before
        )
        decoder_layer_stage3 = TransformerDecoderLayer(
                d_model*4, nhead, dim_feedforward,
                dropout, activation, normalize_before
        )

        decoder_norm_stage1 = nn.LayerNorm(d_model)
        decoder_norm_stage2 = nn.LayerNorm(d_model*2)
    ...
    def forward(self, src):  # src size is [B, C, H, W]
        ...
        hs2 = self.decoder_stage2(tgt_stage2, memories[1].permute(1, 0, 2).contiguous(),
                                  # memory_key_padding_mask=mask,
                                  pos=None,
                                  query_pos=query_embed_stage2)
        hs3 = self.decoder_stage3(tgt_stage3, memories[2].permute(1, 0, 2).contiguous(),
                                  # memory_key_padding_mask=mask,
                                  pos=None,
                                  query_pos=query_embed_stage3)

        outputs_class_stage1 = self.class_embed_stage1(hs1.transpose(1, 2))
        outputs_class_stage2 = self.class_embed_stage2(hs2.transpose(1, 2))
        outputs_class_stage3 = self.class_embed_stage3(hs3.transpose(1, 2))
...
class SetCriterion(nn.Module):
    """ This class computes the loss for PST
    """
    ...
    def loss_area(self, outputs, targets, indices):
        """ Calculate what percentage of the total mask area is occupied by patches 
        that are guessed not to be background, and subtract it from 1 to get the loss.
        """
        pass  # TODO
